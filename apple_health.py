...
def _monthly_score(activities: list[dict], sleep_df: pd.DataFrame, calories_df: pd.DataFrame, rhr_series: pd.Series, days: int) -> dict:
    ...
    # engañoso. Se necesitan al menos 2 de 3 para calcular algo. En Apple
    # Health, recovery_score siempre es None (no hay Training Readiness),
    # así que aquí en la práctica hacen falta sleep_score Y activity_score
    # los dos presentes.
